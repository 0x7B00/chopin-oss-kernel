...
static int awinic_parse_chipid(struct awinic *awinic)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		if (ret < 0) {
			pr_err("%s: failed to read AW_REG_ID: %d\n",
				__func__, ret);
			break;
		}
			    ...
			    const struct i2c_device_id *id)
{
	...
	ret =  create_rb();
	if (ret < 0) {
		pr_info("%s error creating ringbuffer\n", __func__);
		goto err_rb;
	}
...
static int __init awinic_i2c_init(void)
{
	...
	pr_info("%s awinic driver version %s\n", __func__, AW_DRIVER_VERSION);

	ret = i2c_add_driver(&awinic_i2c_driver);
	if (ret) {
		pr_err("%s fail to add awinic device into i2c\n", __func__);
		return ret;
	}
	pr_info("%s awinic_i2c_init success", __func__);
	return 0;
}
